...
public class TimeUtil {
    ...
    public static boolean isValiDate(String dateString,String sourceFormat){
        try {
            DateTimeFormatter fmt = DateTimeFormat.forPattern(sourceFormat);
            DateTime.parse(dateString,fmt);
            return true;
        }catch (Exception e){
            return false;
        }
    ...
    /**
     * 由于filebeat取utc时间，转换其时间戳为本地时间
     *
     * @param dateString
     * @return
     */
    public static String parseTimestampToLocal(String dateString) {
