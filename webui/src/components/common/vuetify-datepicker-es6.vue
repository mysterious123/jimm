...
<template>
  <div class="cov-vue-date">
    <div class="datepickbox ml-3 mr-3">
      <v-text-field
        :readonly="readonly"
          ...
          <div class="cov-picker-box date-list">
            <div class="watch-box">
              ...
              <div class="second-box">
                <div class="mui-pciker-rule mui-pciker-rule-ft"></div>
                <div class="sec-item" v-for="setem in seconds" @click="setTime('second',setem, seconds)"
                     :class="{'active':setem.checked}">{{setem.value}}
                </div>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      getLimitCondition(limit, day) {
        let month = moment(day.moment).format('MM')
        let tmpMoment = moment(day.moment.get('year') + '-' + month + '-' + this.pad(day.value));
        if (limit.from && !limit.to) {
      ...
      showCheck() {
        if (this.dateTime === '') {
          let today = [];
          today.push({
            value: moment(new Date().format('yyyy-MM-dd')).format("DD"),
            inMonth: true,
            unavailable: false,
          ...
          if (this.limit.length > 0) {
            for (let li of this.limit) {
              switch (li.type) {
                case 'fromto':
                  limitDays = this.limitFromTo(li, today)
                  break
                case 'weekday':
                  limitDays = this.limitWeekDay(li, today)
                  break
              }
            }
          }
          //判断当天时间是否在limit范围内
          if ((limitDays.length === 1 && limitDays[0].unavailable === false) || limitDays.length === 0) {
            this.checked.oldtime = moment(new Date().format('yyyy-MM-dd'));//时间默认为当天
          } else {
          ...
          if (this.option.type === 'day' || this.option.type === 'min') {
            this.checked.oldtime = this.dateTime
            this.showDay(this.dateTime)
          } else {
            this.selectedDays = JSON.parse(this.dateTime)
      ...
      picked() {
        if (this.option.type === 'day' || this.option.type === 'min') {
          let ctime = this.checked.year + '-' + this.checked.month + '-' + this.checked.day + ' ' + this.checked.hour + ':' + this.checked.min + ':' + this.checked.second;
          this.checked.currentMoment = moment(ctime, 'YYYY-MM-DD HH:mm:ss')
          this.dateTime = moment(this.checked.currentMoment).format(this.option.format)
        } else {
          this.dateTime = JSON.stringify(this.selectedDays)
        }
      ...
      shiftActTime() {
        ...
        this.$nextTick(() => {
          if (document.querySelector('.hour-item.active')) {
            ...
            document.querySelector('.hour-box').scrollTop = 400;
          }
          if (document.querySelector('.min-item.active')) {
            document.querySelector('.min-box').scrollTop = (document.querySelector('.min-item.active').offsetTop || 0) - 250
          } else {
            document.querySelector('.min-box').scrollTop = 1246;
          }
          if (document.querySelector('.sec-item.active')) {
            document.querySelector('.second-box').scrollTop = (document.querySelector('.sec-item.active').offsetTop || 0) - 250
          } else {
            document.querySelector('.second-box').scrollTop = 1246;
          }
