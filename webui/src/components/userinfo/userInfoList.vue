    ...
    <v-flex xs12>
      <v-card>
        <v-toolbar class="cyan" dark dense>
          <v-toolbar-title class="white--text">联系人</v-toolbar-title>
        </v-toolbar>
        <v-card-title>
          <v-text-field
            append-icon="search"
          ...
          ></v-text-field>
          <v-spacer></v-spacer>
          <v-btn icon @click="pagerouter({name:'userInfoAdd'})" v-if="isPermitted('userinfo/insert')">
            <v-icon>add</v-icon>
          </v-btn>
          <v-btn icon @click="updateUserInfo()" v-if="isPermitted('userinfo/update')">
            <v-icon>edit</v-icon>
          </v-btn>
          <v-btn icon @click="delUserInfo()" v-if="isPermitted('userinfo/delete')">
        ...
        >
          ...
          <template slot="items" slot-scope="props">
            <tr :active="props.selected" align="center" @click="props.selected = !props.selected">
              ...
              <!--<td>{{props.item.manageGroups}}</td>-->
              <td>{{props.item.phone}}</td>
              <td>{{props.item.email}}</td>
              <td>{{getDate(props.item.createDate)}}</td>
            </tr>
          </template>
        </v-data-table>
      </v-card>
...
//          },
    ...
    methods: {
      ...
      queryItems() {
        ...
        axios().get("/userinfo/queryGroupName").then(function (response) {
      ...
      updateUserInfo(){
        if (this.selected.length === 1) {
          this.setUpdateUserInfo(this.selected[0]);
          this.pagerouter({name: 'userInfoEdit'});
        } else {
