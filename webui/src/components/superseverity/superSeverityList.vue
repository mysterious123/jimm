    ...
    <v-flex xs12>
      <v-card>
        <v-toolbar class="cyan" dark dense>
          <v-toolbar-title class="white--text">级别信息</v-toolbar-title>
        </v-toolbar>
        <v-card-title>
          <v-text-field
            append-icon="search"
          ...
          ></v-text-field>
          <v-spacer></v-spacer>
          <v-btn icon @click="pagerouter({name:'superSeverityAdd'})">
            <v-icon>add</v-icon>
          </v-btn>
          <!--<v-btn icon @click="updateSuperSeverity()">-->
          <!--<v-icon>edit</v-icon>-->
          <!--</v-btn>-->
          <v-btn icon @click="delSuperSeverity()">
        ...
        >
          ...
          <template slot="items" slot-scope="props">
            <tr :active="props.selected" align="center" @click="props.selected = !props.selected" :bgcolor="props.item.levelColour">
              <td>
                <v-checkbox
                  primary
                  hide-details
              ...
              <td>{{props.item.chineseName}}</td>
              <td >
                {{props.item.levelColour}}
              </td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapMutations('superseverity', [
        'setUpdateSuperSeverity',
        'clearUpdateSuperSeverity'
      ]),
      //加载表格数据
      queryItems() {
        var vm = this;
        vm.items_loading = true;
        axios().get("/superSeverity").then(function (response) {
      ...
      updateSuperSeverity(){
        if (this.selected.length === 1) {
          this.setUpdateSuperSeverity(this.selected[0]);
          this.pagerouter({name: 'superSeverityEdit'});
        } else {
