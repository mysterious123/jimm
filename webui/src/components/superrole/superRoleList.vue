    ...
    <v-flex xs12>
      <v-card>
        <v-toolbar class="cyan" dark dense>
          <v-toolbar-title class="white--text">用户角色</v-toolbar-title>
        </v-toolbar>
        <v-card-title>
          <v-text-field
            append-icon="search"
          ...
          ></v-text-field>
          <v-spacer></v-spacer>
          <v-btn icon @click="pagerouter({name:'superRoleAdd'})">
            <v-icon>add</v-icon>
          </v-btn>
          <v-btn icon @click="updateSuperRole()">
            <v-icon>edit</v-icon>
          </v-btn>
          <v-btn icon @click="delSuperRole()">
...
<script>
  ...
  export default {
    ...
    methods : {
     ...mapMutations('superrole', [
       'setUpdateSuperRole',
       'clearUpdateSuperRole'
     ]),
     //加载表格数据
     queryItems() {
       var vm = this;
       vm.items_loading = true;
       axios().get("/superRole").then(function (response) {
     ...
     updateSuperRole(){
        if(this.selected.length === 1){
          this.setUpdateSuperRole(this.selected[0]);
          this.pagerouter({name:'superRoleEdit'});
        }else{
