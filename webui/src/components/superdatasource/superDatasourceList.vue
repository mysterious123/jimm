    ...
    <v-flex xs12>
      <v-card>
        <v-toolbar class="cyan" dark dense>
          <v-toolbar-title class="white--text">数据源管理</v-toolbar-title>
        </v-toolbar>
        <v-card-title>
          <v-text-field
            append-icon="search"
          ...
          ></v-text-field>
          <v-spacer></v-spacer>
          <v-btn icon @click="pagerouter({name:'superDatasourceAdd'})">
            <v-icon>add</v-icon>
          </v-btn>
          <!--<v-btn icon @click="updateSuperDatasource()">-->
          <!--<v-icon>edit</v-icon>-->
          <!--</v-btn>-->
          <v-btn icon @click="delSuperDatasource()">
        ...
        >
          ...
          <template slot="items" slot-scope="props">
            <tr :active="props.selected" align="center" @click="props.selected = !props.selected">
              ...
              <td>{{props.item.dbType}}</td>
              <td>{{props.item.serverIp}}</td>
              <td>{{props.item.databaseName}}</td>
              <td>{{props.item.portNum}}</td>
...
<script>
  ...
  export default {
    mounted() {
      this.queryItems();
      this.clearUpdateSuperDatasource();
    },
    data() {
      return {
        search: "", //页面查询模型
    ...
    methods: {
      ...mapMutations('superdatasource', [
        'setUpdateSuperDatasource',
        'clearUpdateSuperDatasource'
      ]),
      //加载表格数据
      queryItems() {
        var vm = this;
        vm.items_loading = true;
        axios().get("/superDatasource").then(function (response) {
          vm.items = response.data;
          vm.items_loading = false;
        })
      },
      updateSuperDatasource(){
        if (this.selected.length === 1) {
          this.setUpdateSuperDatasource(this.selected[0]);
          this.pagerouter({name: 'superDatasourceEdit'});
        } else {
