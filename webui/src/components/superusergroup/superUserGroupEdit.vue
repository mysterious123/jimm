<template>
  <v-card>
    <v-toolbar class="cyan" dark dense>
      ...
      <v-toolbar-title class="white--text">用户组编辑</v-toolbar-title>
    </v-toolbar>
    <v-card-text>
      <v-layout row wrap>
        <v-flex xs12>
    <v-text-field
       label="用户组名称"
       v-model="updateSuperUserGroup.groupName"
       v-validate="'required'"
       :error-messages="vee_errors.collect('用户组名称','superusergroup_update')[0]||[]"
       data-vv-name="用户组名称"
       data-vv-scope="superusergroup_update"
     ></v-text-field>
    <v-text-field
       label="角色名"
       v-model="updateSuperUserGroup.roleId"
       data-vv-name="角色名"
       data-vv-scope="superusergroup_update"
     ></v-text-field>
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="success" dark @click.stop="superusergroup_update()">
        <v-icon>check</v-icon>
        更新
      </v-btn>
    </v-card-actions>
...
<script>
  ...
  export default {
    beforeRouteEnter (to, from, next) {
      if(!store.state.superusergroup.updateSuperUserGroup) {
        next({name : 'superUserGroup'})
      }else{
        next();
      }
    },
    ...
    mounted(){
      axios().get("superUserGroup/querySelectUser",{
        params : {
          id : this.updateSuperUserGroup.id
        }
      }).then((data)=>{
        this.selectUsers = data.data
      })
      axios().get("superUserGroup/queryNoSelectUser",{
        params : {
          id : this.updateSuperUserGroup.id
        }
      }).then((data)=>{
    ...
    computed: {
      ...mapState('superusergroup', ['updateSuperUserGroup']),
      superUserIds(){
    ...
    methods:{
      superusergroup_update(){
        this.$validator.validateAll("superusergroup_update").then((result) => {
          if (result) {
            this.updateSuperUserGroup.superUserIds = this.superUserIds;
            axios().put("/superUserGroup",this.updateSuperUserGroup).then(()=>{
